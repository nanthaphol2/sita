...
export default function AuctionForm() {
  ...
  async function onSubmit(data: FieldValues) {
    try {
      ...
      router.push(`/auctions/details/${res}`)
  } catch (error: any) {
      console.log(error);
  }
  }
  ...
  return (
    <form className='flex flex-col mt-3' onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='grid grid-cols-2 gap-3'>
            <Input label='Reverse price (enter 0 is not reverse)' name='reversePrice' control={control} type='number' rules={{ required: 'Reverse price is required' }} /> 
